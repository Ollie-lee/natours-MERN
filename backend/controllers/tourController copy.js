...
exports.getAllTours = async (req, res) => {
  try {
    ...
    //by default, we want page number one and send limit 100 document
    ...
    //when specified page number > having page number
    //only happens when specified page number
    if (req.query.page) {
      //return the amount of documents as the resolved value of a promise
      const numTours = await Tour.countDocuments();
      //have skipped all documents in the collection
      //be careful with equal,
      // e.g.all documents = 9, specified page=4,limit=3,should error, but skip = 9
      if (skip >= numTours) {
    ...
    //execute query
    //query.sort().select().skip().limit(), then using await, return an array of documents
